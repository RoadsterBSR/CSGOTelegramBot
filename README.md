...
2. Open the solution and build it to download and install NuGet packages. Notice: **CSGOTelegramData** contains pre-build scripts to copy needed files to the output folder. If you changed projectnames, you need to adjust these scripts accordingly before building the solution.
3. Open **CSGOTelegramData** project and edit in **Config.cs** the variables:  
  ...
  c. **SteamWebapiKey** (Your personal registered SteamWebApi key. See: [SteamCommunity](https://steamcommunity.com/dev/apikey))   
  d. (Optional) Set the **MainTitle** and **BotVersion** to anything you want  
  e. **ServerIP** (IPAddress of the CSGO gameserver you want to retrieve info from)  
  f. **ServerPort** (Network port of the CSGO gameserver you want to retrieve info from)  
  g. (Optional) Fill the **Admins** list with the steamID64's of the players who are an admin on the CSGO gameserver  
  h. (Optional) Fill the  **Clans** list in the clan section  
